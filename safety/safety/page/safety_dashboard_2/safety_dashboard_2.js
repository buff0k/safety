frappe.pages["safety-dashboard-2"].on_page_load = function (wrapper) {
  const page = frappe.ui.make_app_page({
    parent: wrapper,
    title: "Safety Dashboard 2",
    single_column: true
  });
  ...
  async function load() {
    ...
    const r = await frappe.call({
      method: "safety.safety.report.head_office_safe_days.head_office_safe_days.get_today_snapshot",
      args: { filters: {} }
    });
    ...
    const headOfficeColor = (payload.head_office_color || "").trim();

    const headOfficeRow = rows["Head Office"] || null;

    // Group sites by complex
    const groups = {};
    Object.keys(rows)
      .filter(site => site && site !== "Head Office")
      .forEach(site => {
    ...
    top.appendChild(render_head_office_card(headOfficeRow, headOfficeColor));
...
function render_head_office_card(row, headOfficeColor) {
  const card = document.createElement("div");
  card.className = "isd-card isd-company";

  if (headOfficeColor && is_valid_hex(headOfficeColor)) card.style.background = headOfficeColor;
