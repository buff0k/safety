		...
		cols.append(
			{
				"label": str(d),
				"fieldname": f"d{d:02d}",
				"fieldtype": "Data",  # weekday rows are text; data rows are ints
				"width": 45,
			}
		)
...
def get_data(filters: dict) -> list[dict]:
	...
	# One query returns both series by day
	rows = frappe.db.sql(
		f"""
		SELECT
	...
	# Month blocks (Jan..Dec for each year like the spreadsheet)
	for yy in years:
	...
	# Bottom totals (Total TMM, Total Injury)
...
def _weekday_row(yy: int, mm: int) -> dict:
	...
	for d in DAY_COLS:
		if d <= last_day:
			row[f"d{d:02d}"] = calendar.day_abbr[date(yy, mm, d).weekday()]  # Mon/Tue/...
		else:
			row[f"d{d:02d}"] = ""
...
def _year_total_row(
	...
	for d in DAY_COLS:
		...
		for mm in range(1, 13):
			last_day = calendar.monthrange(yy, mm)[1]
			if d <= last_day:
				current = date(yy, mm, d)
				if y_start <= current <= y_end:
...
def get_chart(data: list[dict]) -> dict:
	"""
	Chart: totals per day-of-month from bottom Total rows + Total bar.
	"""
	if not data:
		return {}
